...
from tools.transform import bv_anchor_to_lidar
DEBUG = False

def anchor_target_layer(rpn_cls_score, gt_boxes, gt_boxes_3d, im_info, _feat_stride=[8, 8]):
    """
    ...
    anchors = np.hstack((labels_new.reshape(-1,1), anchors[all_inds])).astype(np.float32)
...
def bbox_transform_3d(ex_rois_3d, gt_rois_3d):
    ...
    targets = np.vstack((targets_dx, targets_dy, targets_dz)).transpose()
    # targets_dl = np.log(gt_lengths / ex_lengths)
    # targets_dw = np.log(gt_widths / ex_widths)
    # targets_dh = np.log(gt_heights / ex_heights)
    # targets = np.vstack((targets_dx, targets_dy, targets_dz, targets_dl, targets_dw, targets_dh)).transpose()
    # targets = np.vstack(
    #     (targets_dx, targets_dy, targets_dl, targets_dw)).transpose()
    return targets
